...
Kelly.addCommand({pattern: 'filter ?(.*)', fromMe: true, desc: Lang.FILTER_DESC, dontAddCommandList: true}, (async (message, match) => {
...
Kelly.addCommand({pattern: 'stop ?(.*)', fromMe: true, desc: Lang.STOP_DESC, dontAddCommandList: true}, (async (message, match) => {
    match = match[1].match(/[\'\"\“](.*?)[\'\"\“]/gsm);
    if (match === null) {
...
Kelly.addCommand({on: 'text', fromMe: false}, (async (message, match) => {
        if(Config.BGMFILTER){
...
Kelly.addCommand({on: 'text', fromMe: false,onlyGroup: true}, (async (message, match) => {
...
filtreler.map(
 ...
 Kelly.addCommand({on: 'text', fromMe: false, onlyPm: true}, (async (message, match) => {
